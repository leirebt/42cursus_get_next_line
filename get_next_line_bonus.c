...
/*   Updated: 2021/12/01 19:20:08 by lbarture         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*holder[7000];
	char		*line;

	line = 0;
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	holder[fd] = read_fd(holder[fd], fd, BUFFER_SIZE);
	if (!holder[fd])
		return (0);
	line = create_line (holder[fd]);
	holder[fd] = update_holder(holder[fd], line);
	return (line);
}
